...
private:
	...
	string role;
public:
	Employee(string eno = "---", string role = "---", string pid = "---", string pname = "---") : Person(pid, pname), eno(eno), role(role) {}
	~Employee() {};
...
// 인자로 넘어온 Employee 포인터와 같은지 확인.
// 내가 작성
bool Employee::Equals(Employee* p) {
	return this->eno == p->eno;
}
// Employee 객체의 내용 print
// 내가 작성
void Employee::Print() {
	cout << "| " << this->eno << " | " << this->role << " |";
}
class Student {
private:
	string sid;
...
class RowRecords {
public:
	RowRecords(int maxKeys = 5, int unique = 1) : NumKeys(0), Keys(0), Records(0) {
		Init(maxKeys, unique);
	}
// unique = 1은 중복이 없다
	~RowRecords();
...
int RowRecords<T>::Insert(const string key, const T & record) {
	...
	if (NumKeys == MaxKeys) throw OverflowException(); // 이미 Records가 가득 차있다면 throw OverflowException 인데 오타인듯?
	...
	Keys[i + 1] = key; // Keys[i]
	Records[i + 1] = record; // Records[i] 인데 잘못쓴듯?

	NumKeys++;
	return 1;
}
template <class T>
// Remove 하는데 T& record 인자는 필요없다고 생각해서 지움.
int RowRecords<T>::Remove(const string key) {
	int index = Find(key); // 지우고자 하는 key에 해당하는 index
...
public:
	...
	//void Print(ostream&) const;
	void Print() const;
	RecordTable(int keySize, int maxKeys = 5, int maxRecords = 10) : MaxKeys(maxKeys), MaxRecords(maxRecords) {
		NumRecords = 0;
...
// 못찾으면 throw NotFoundRecordException() 
...
int RecordTable<T>::Append(const T & record) {
	...
	// if (NumRecords == MaxRecords) 일경우도 처리해줘야할듯?
	int result = data[i]->Insert(key, record); //key랑 record 둘다 줘야됨.
	return result;
}
